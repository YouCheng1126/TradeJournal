...
import { AreaChart, Area, BarChart, Bar, Cell, ResponsiveContainer, XAxis, YAxis, Tooltip, CartesianGrid, ReferenceLine } from 'recharts';
...
export const CumulativePnlChart: React.FC<CumulativePnlChartProps> = ({ trades, commission }) => {
    ...
    const [metric, setMetric] = useState<MetricType>('cum_pnl'); 
            ...
            <div className="flex items-center justify-between mb-4 relative z-20">
                <div className="flex items-center gap-3">
                    <div className="flex bg-slate-800/50 rounded-lg p-0.5 border border-slate-600 w-20">
                        <button onClick={() => setChartType('line')} className={getToggleClass(chartType === 'line')} title="Line Chart"><TrendingUp size={14} /></button>
                        <button onClick={() => setChartType('bar')} className={getToggleClass(chartType === 'bar')} title="Bar Chart"><BarChart3 size={14} /></button>
                    </div>
                ...
                <div className="flex bg-slate-800/50 rounded-lg p-0.5 border border-slate-600 w-32">
                    <button onClick={() => setTimeframe('day')} className={getToggleClass(timeframe === 'day')}>Day</button>
                    <button onClick={() => setTimeframe('month')} className={getToggleClass(timeframe === 'month')}>Month</button>
                </div>
            </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                    {chartType === 'line' ? (
                        <AreaChart data={data} margin={{ top: 10, right: 10, left: -5, bottom: 0 }}>
                            <defs>
                                <linearGradient id="splitColorLeft" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <YAxis stroke="#94a3b8" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(val) => isCurrency ? `$${val}` : `${val}`} ticks={ticks} domain={domain as [number, number]} width={45} interval={0} />
                            <Tooltip 
                                position={{ y: 50 }} 
                                cursor={<CustomCursor />}
                            ...
                            <Area type="monotone" dataKey="value" stroke="#8b5cf6" strokeWidth={2} fill="url(#splitColorLeft)" activeDot={<CustomActiveDot />} />
                        </AreaChart>
                    ) : (
