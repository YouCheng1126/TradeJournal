...
import { LayoutDashboard, BookOpen, PieChart, FilePlus2, Database, Filter, Calendar as CalendarIcon, BookMarked, Tag as TagIcon, Settings } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, onOpenAddModal, onOpenTagManager, onOpenUserSettings }) => {
  ...
  return (
    // Changed h-screen to h-full w-full to fill the zoomed body container correctly
    <div className="flex w-full h-full bg-background text-text overflow-hidden">
      {/* Sidebar */}
      <aside className="w-64 bg-surface border-r border-slate-700 flex flex-col hidden md:flex flex-shrink-0">
        <div className="p-6">
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        {/* Header */}
        <header className="h-16 bg-surface/50 border-b border-slate-700 flex items-center justify-between px-6 backdrop-blur-md flex-shrink-0">
           {/* Filters moved here */}
